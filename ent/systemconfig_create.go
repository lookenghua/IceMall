...
func (scc *SystemConfigCreate) SetRule(s string) *SystemConfigCreate {
	scc.mutation.SetRule(s)
	return scc
}
...
func (scc *SystemConfigCreate) check() error {
	if _, ok := scc.mutation.Rule(); !ok {
		return &ValidationError{Name: "rule", err: errors.New(`ent: missing required field "SystemConfig.rule"`)}
	}
	...
	if _, ok := scc.mutation.Remark(); !ok {
...
func (scc *SystemConfigCreate) createSpec() (*SystemConfig, *sqlgraph.CreateSpec) {
	...
	if value, ok := scc.mutation.Rule(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeString,
			Value:  value,
			Column: systemconfig.FieldRule,
		})
		_node.Rule = value
	}
	if value, ok := scc.mutation.Value(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeString,
			Value:  value,
			Column: systemconfig.FieldValue,
		})
		_node.Value = value
	}
	if value, ok := scc.mutation.Remark(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeString,
			Value:  value,
			Column: systemconfig.FieldRemark,
		})
		_node.Remark = value
	}
	return _node, _spec
